        ...
        {open && (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <button type="button" onClick={() => setViewMonth(m => subMonths(m, 1))} className="w-7 h-7 border border-border-strong rounded-xs bg-transparent text-text-muted flex items-center justify-center cursor-pointer hover:text-white transition-colors">
                <ChevronLeft size={14} />
              </button>
              <span className="font-heading font-bold text-sm capitalize">{format(viewMonth, 'MMMM yyyy', { locale: es })}</span>
              <button type="button" onClick={() => setViewMonth(m => addMonths(m, 1))} className="w-7 h-7 border border-border-strong rounded-xs bg-transparent text-text-muted flex items-center justify-center cursor-pointer hover:text-white transition-colors">
                <ChevronRight size={14} />
              </button>
            </div>

            <div className="grid grid-cols-7 gap-0.5 mb-1">
              {WEEKDAYS.map(d => <div key={d} className="text-center text-[10px] font-bold text-text-muted py-1 uppercase">{d}</div>)}
            </div>
