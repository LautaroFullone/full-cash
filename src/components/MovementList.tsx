          ...
          {filters.map(f => (
            <button
              ...
              className={cn(
                ...
                filter === f.value ? 'bg-surface-elevated text-white' : 'bg-transparent text-text-muted'
              )}
            >
              {f.label}
            </button>
        ...
        <div className="flex flex-col gap-0.5">
          {filtered.map(mov => (
            ...
            >
              {/* Category icon */}
              <div className={cn(
                'w-[38px] h-[38px] rounded-sm shrink-0 flex items-center justify-center',
                mov.tipo === 'INGRESO' ? 'bg-accent/10 text-accent' : 'bg-danger/10 text-danger'
              )}>
