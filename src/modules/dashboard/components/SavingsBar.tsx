  ...
  return (
    <div className="card animate-slide-up p-5 [animation-delay:0.2s] [animation-fill-mode:backwards]">
      <div className="flex items-center justify-between mb-3.5">
        ...
        {editing ? (
          <div className="flex items-center gap-2">
            <input type="number" min={1} max={100} value={tempPorcentaje} onChange={e => setTempPorcentaje(Number(e.target.value))} className="w-[60px] text-center text-[13px] rounded-sm py-1.5 px-2" />
            <span className="text-[13px] text-text-muted">%</span>
            <button onClick={handleSave} className="w-7 h-7 rounded-sm border-none bg-accent text-background flex items-center justify-center">
              <Check size={14} />
            </button>
          </div>
        ) : (
          <button onClick={() => { setTempPorcentaje(Math.round(porcentajeAhorro * 100)); setEditing(true) }} className="w-7 h-7 rounded-sm border border-border-strong bg-transparent text-text-muted flex items-center justify-center hover:border-accent hover:text-accent transition-all duration-200">
            <Settings2 size={14} />
          </button>
        )}
      </div>
      ...
      <div className="h-2 rounded-full bg-white/6 overflow-hidden mb-3">
        <div className="h-full rounded-full transition-all duration-500" style={{ width: `${Math.max(progreso, 0)}%`, background: progressColor }} />
      </div>

      <div className="flex justify-between text-[13px]">
        <span className="text-text-muted">Ahorro real: <span className={`font-semibold ${ahorroReal >= 0 ? 'text-accent' : 'text-danger'}`}>{formatCurrency(ahorroReal)}</span></span>
        <span className="text-text-muted">Meta: <span className="font-semibold text-text-secondary">{formatCurrency(montoRecomendado)}</span></span>
      </div>
    </div>
  )
}
