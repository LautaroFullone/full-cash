...
export function MovementForm({ categorias, plataformas, onSubmit, isOpen: controlledOpen, onClose: controlledClose, onOpen: controlledOnOpen }: MovementFormProps) {
  ...
  const resetForm = () => {
    setConcepto(''); setMonto(''); setCategoriaId(''); setPlataformaId('')
    setFecha(format(new Date(), 'yyyy-MM-dd')); setError('')
  }
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="modal-overlay fixed inset-0 bg-black/65 backdrop-blur-md z-50 flex justify-center" onClick={handleClose}>
          <div
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-[18px]">
              ...
              <div>
                ...
                <input value={concepto} onChange={e => setConcepto(e.target.value)} placeholder="Ej: Sueldo, Supermercado..." className="w-full" />
              </div>
              ...
              <div>
                ...
                {filteredCategorias.length === 0 ? (
                  <p className="text-[13px] text-text-muted py-2">No hay categorías de {tipo === 'INGRESO' ? 'ingreso' : 'gasto'}.</p>
                ) : (
