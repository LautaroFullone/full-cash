...
export function useCategories() {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: PutCategoriaBody }) => putCategoria(id, data),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['categorias'] }),
  })
  ...
  return {
    ...
    updateCategoria: (id: string, data: PutCategoriaBody) => updateMutation.mutateAsync({ id, data }),
